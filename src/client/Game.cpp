...
D3DCOLOR g_dwCOLOR[] = { g_dwRED,    g_dwGREEN,  g_dwBLUE, g_dwBLACK,
                         g_dwWHITE,  g_dwYELLOW, g_dwGRAY, g_dwVIOLET,
                         g_dwORANGE, g_dwPINK };
...
{
  int iLostExp = ((iGetExp + 20) / (g_pAVATAR->Get_LEVEL() + 4) + 5) *
                 (m_iWorldStaminaVal) / 100;
