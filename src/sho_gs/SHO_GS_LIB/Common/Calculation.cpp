...
bool CCal::Get_DropITEM(int iLevelDiff, CObjMOB* pMobCHAR, tagITEM& sITEM, int iZoneNO, int iDropRate, int iCharm)
{
	...
	if (iDrop_VAR <= 0) {
		// 드롭 확률 저조 !!! 생성안됨. = Low drop probability !!!Not created.
		return false;
	}

	if (1 + RANDOM(100) <= NPC_DROP_MONEY(pMobCHAR->Get_CharNO())) {
		// 돈생성 = Money generation
	...
	if (iDropITEM <= 1000) {
		if (iDropITEM >= 1 && iDropITEM <= 4) {
			...
			if (iDropTblIDX >= g_TblDropITEM.m_nColCnt) {
				// 테이블 컬럼 갯수 초과... = Number of table columns exceeded
				return false;
			}
	...
	if (sITEM.GetTYPE() >= ITEM_TYPE_ETC && sITEM.GetTYPE() != ITEM_TYPE_RIDE_PART) { // ITEM_TYPE_RIDE_PART은 제외해야 함
		...
		if (sITEM.GetTYPE() >= ITEM_TYPE_USE && sITEM.GetTYPE() != ITEM_TYPE_RIDE_PART) { // ITEM_TYPE_RIDE_PART은 제외해야 함
			...
			switch (nRareType) {
			case 3:	// 대만 레어 아이템 = Taiwan rare items
			...
			case 0:
			{
				...
				if (sITEM.GetTYPE() != ITEM_TYPE_JEWEL) {
					// 장비는 각 stb의 기본 품질 값을 설정. = The equipment sets the default quality value for each stb.
			...
			// 내구도 결정 = Determination of durability
			...
			{
				// 제련등급 결정. = Determination of smelting grade.
				iTEMP = 1 + RANDOM(100);
...
//-------------------------------------------------------------------------------------------------
// iGiveDamage = 공격자가준 데미지 = Attacker's Damage
...
//-------------------------------------------------------------------------------------------------
// 크리티컬 Success 값 공식 2005-7-13 추가 = Added Success value formula 2005-7-13
...
#else
	return (int)(((1 + RANDOM(100)) * 3 + pATK->Get_LEVEL() + 30) * 16 / (pATK->Get_CRITICAL() + 70));
#endif
...
//-------------------------------------------------------------------------------------------------
// 일반 물리 데미지 계산식... = General Physics Damage Formula
WORD CCal::Get_BasicDAMAGE(CObjCHAR* pATK, CObjCHAR* pDEF, WORD wHitCNT, int iSuc)
{
	...
	// 맞는 동작 확률. = Probability of operation.
	int iHitActRATE = (28 - iCriSuc) * (pATK->Get_ATK() + 20) / (pDEF->Get_DEF() + 5);

	if (pATK->IsUSER()) {
		LogString(LOG_NORMAL, "Get_BasicDamage for player - HitRate = %d - Crit suc = %d", iHitActRATE, iCriSuc);
	}
	else {
		LogString(LOG_NORMAL, "Get_BasicDamage for monster - HitRate = %d - Crit suc = %d", iHitActRATE, iCriSuc);
	}
	...
	if (iCriSuc < 20) {
		...
		// 추가 데미지 스킬 ... = Additional damage skills ...
	...
	LogString(LOG_NORMAL, "GS/Calculation::Get_BasicDamage - Calculated damage = %i", iDamage);
...
WORD CCal::Get_SkillDAMAGE(CObjCHAR* pATK, CObjCHAR* pDEF, short nSkillIDX, WORD wHitCNT)
{
	...
	// int iSuccess  = Get_SuccessRATE (pATK, pDEF); // 2005-7-26 kchs 수정. 잘못되서 다시 옛날로 복원 = 2005-7-26 kchs modified. Wrong and restore back to old
	...
	default:
	{
		// 일반 공격 데미지로 계산... = Calculated as normal attack damage
		int iSuccess;
		int iRAND1 = 1 + RANDOM(80);
		int iRAND2 = 1 + RANDOM(50);
